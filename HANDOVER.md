...
Every exit — stop button, fault, stalled stream, Ctrl-C — walks both arms to
the park pose rather than dropping them.
...
Do the same for the **park pose** (`deploy.home_joint_position_rad`) — the
folded pose each arm rests in on exit. The shipped default is all zeros; the
YAM's j2/j3 lower limits are *exactly* 0.0, so zeros is the folded pose but sits
precisely on two limits. Verify that is right for your arms.
...
i2rt's `MotorChainRobot.close()` sets all torques to zero. Teleop parks to
`deploy.home_joint_position_rad` first and *then* closes, so the park pose has
to be a pose the arm can physically rest in **unpowered**. If it is not, the arm
sags or drops the moment you Ctrl-C.

The shipped default (all zeros) is low and tucked in against the base — ee at
roughly (0.11, 0.00, 0.17) m — which is plausibly stable, but confirm it on your
arm by hand before trusting it to a shutdown. The same applies to any park pose
you record.
